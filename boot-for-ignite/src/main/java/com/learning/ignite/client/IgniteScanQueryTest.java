/**
 * Author:   claire
 * Date:    2021-04-06 - 10:05
 * Description: scan-query-test
 * History:
 * <author>          <time>                   <version>          <desc>
 * claire          2021-04-06 - 10:05          V1.17.0          scan-query-test
 */
package com.learning.ignite.client;

import com.learning.ignite.entity.Organization;
import com.learning.ignite.entity.Person;
import com.learning.ignite.entity.PersonKey;
 ...
 * @since 1.1.0
 */
public class IgniteScanQueryTest {
    private static final String ORG_CACHE_NAME = IgniteScanQueryTest.class.getSimpleName() + "-organizations";

    /**
     * Partitioned cache name (to store employees).
     */
    private static final String PERSON_CACHE_NAME = IgniteScanQueryTest.class.getSimpleName() + "-persons";
    ...
    public static void main(String[] args) throws InterruptedException {
        try (Ignite ignite = Ignition.start("config/ignite.xml")) {
            ...
            try (
                ...
                Query<Cache.Entry<Integer, Organization>> qryOrg = new ScanQuery<Integer, Organization>(
                        (i, p) -> p.getName().contains("Other"));

                try (QueryCursor<Cache.Entry<Integer, Organization>> cur = orgCache.query(qryOrg)) {
                    for (Cache.Entry<Integer, Organization> entry : cur) {
                        // Process the entry ...
